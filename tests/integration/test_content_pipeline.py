...
class TestContentPipelineIntegration:
    ...
    def test_content_upload_with_validation(self, client: TestClient, auth_headers: dict):
        ...
        assert data["validation_status"] in ["passed", "pending"]
    ...
    def test_content_cultural_adaptation(self, client: TestClient, auth_headers: dict):
        ...
        response = client.post(
            f"/api/v1/content/{content_id}/adapt-cultural",
            json={
                "region": "south",
    ...
    def test_content_grade_adaptation(self, client: TestClient, auth_headers: dict):
        ...
        response = client.post(
            f"/api/v1/content/{content_id}/adapt-grade",
            json={
                "target_grade": 8,
    ...
    def test_content_translation_pipeline(self, client: TestClient, auth_headers: dict):
        ...
        # Translate content
        response = client.post(
            f"/api/v1/content/{content_id}/translate",
            json={
                "target_language": "hi",
    ...
    def test_full_pipeline_with_all_services(self, client: TestClient, auth_headers: dict):
        ...
        response = client.post(
            f"/api/v1/content/{content_id}/generate-audio",
            json={"language": "hi"},
            headers=auth_headers
        )
        assert response.status_code == 200
...
class TestABTestingIntegration:
    ...
    def test_experiment_creation_and_assignment(self, client: TestClient, auth_headers: dict):
    ...
    def test_experiment_event_tracking(self, client: TestClient, auth_headers: dict):
        ...
        response = client.post("/api/v1/experiments/", json=experiment_data, headers=auth_headers)
        experiment_id = response.json()["id"]
        
        client.post(f"/api/v1/experiments/{experiment_id}/start", headers=auth_headers)
        
        # Track impression
        response = client.post(
            f"/api/v1/experiments/{experiment_id}/track",
            json={
                "user_id": "test_user_1",
...
@pytest.mark.skip(reason="Rate limiting tests require Redis and properly configured auth tokens")
class TestRateLimitingIntegration:
    """Test rate limiting across different user roles."""
    
    
    def test_student_rate_limit(self, client: TestClient):
        """Test student rate limits (60/min)."""
        # Create student user
        ...
        success_count = 0
        for i in range(65):
            response = client.get("/api/v1/content/", headers=student_headers)
    ...
    def test_teacher_rate_limit(self, client: TestClient):
        """Test teacher rate limits (200/min)."""
        teacher_headers = {"Authorization": "Bearer teacher_token"}
        
        # Teachers should handle more requests
        success_count = 0
        for i in range(100):
            response = client.get("/api/v1/content/", headers=teacher_headers)
            if response.status_code == 200:
                success_count += 1
        
        assert success_count == 100  # Should not hit limit
...
class TestEndToEndScenarios:
    ...
    def test_teacher_content_creation_workflow(self, client: TestClient):
        """Test complete teacher workflow: login → create → validate → publish."""
        # Step 1: Teacher login
        response = client.post(
            "/api/v1/auth/login",
            json={"username": "teacher@example.com", "password": "test123"}
        )
        assert response.status_code == 200
        ...
        # Step 3: Validate curriculum alignment
        response = client.post(
            f"/api/v1/content/{content_id}/validate",
            headers=headers
        )
        assert response.status_code == 200
    ...
    def test_student_learning_workflow(self, client: TestClient):
        """Test complete student workflow: login → browse → learn → quiz."""
        # Student login
        response = client.post(
            "/api/v1/auth/login",
            json={"username": "student@example.com", "password": "test123"}
        )
        assert response.status_code == 200
        token = response.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        ...
        response = client.get(
            "/api/v1/content/",
            params={"grade_level": 6, "subject": "science"},
            headers=headers
        )
        assert response.status_code == 200
        ...
        response = client.get(
            f"/api/v1/content/{content_id}/audio",
            headers=headers
        )
        assert response.status_code in [200, 404]  # 404 if not generated yet
        
        # Take quiz
        response = client.post(
            f"/api/v1/content/{content_id}/quiz",
            json={"answers": [{"question_id": 1, "answer": "A"}]},
            headers=headers
        )
        assert response.status_code in [200, 404]  # 404 if no quiz available
    
    
    def test_multilingual_content_delivery(self, client: TestClient, auth_headers: dict):
        """Test delivering content in multiple languages."""
        ...
        assert response.status_code == 200
        tamil_content = response.json()
        
        # Verify different translations
        assert hindi_content["content"] != tamil_content["content"]

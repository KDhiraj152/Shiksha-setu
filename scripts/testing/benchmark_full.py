...
class BenchmarkSuite:
    ...
    def add_result(self, result: BenchmarkResult):
        ...
        print(f"  {'✓' if result.status == 'PASS' else '✗'} {result.name}: {result.value:.2f} {result.unit}")
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            for i in range(100):
                ...
                conn.execute(text(
                    "INSERT INTO bench_vectors (embedding) VALUES (:vec::vector)"
                ), {"vec": str(vec)})
            conn.commit()
...
def benchmark_codebase(suite: BenchmarkSuite):
    ...
    ts_files = list((PROJECT_ROOT / "frontend" / "src").glob("**/*.tsx")) + \
               list((PROJECT_ROOT / "frontend" / "src").glob("**/*.ts"))
...
def run_benchmarks():
    ...
    suite.summary = {
        ...
        "failed": failed,
        "skipped": skipped,
        "success_rate": f"{(passed / len(suite.results)) * 100:.1f}%" if suite.results else "0%"
    }

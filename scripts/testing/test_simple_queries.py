...
def test_intent_classification():
    ...
    for query, expected_intent in test_cases:
        ...
        if result.primary_intent == expected_intent:
            print(f"✅ '{query}' → {result.primary_intent.value} (confidence: {result.confidence})")
        else:
            print(f"❌ '{query}' → got {result.primary_intent.value}, expected {expected_intent.value}")
...
def test_is_simple_query():
    ...
    test_cases = [
        ...
        ("explain the theory of relativity in detail with examples", Intent.EXPLANATION, False),
    ]
...
def test_formatter_intent_detection():
    ...
    from backend.services.ai_core.formatter import ResponseFormatter, Intent
    ...
    for query, expected_intent in test_cases:
        ...
        if result == expected_intent or result != Intent.UNKNOWN:
            ...
            print(f"❌ '{query}' → got {result.value}, expected {expected_intent.value}")
...
async def test_full_chat_flow():
    ...
    from backend.services.ai_core.engine import get_ai_engine, GenerationConfig

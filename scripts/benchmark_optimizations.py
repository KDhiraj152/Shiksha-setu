...
def benchmark(name: str, func, iterations: int = 10000, warmup: int = 100) -> BenchmarkResult:
...
async def benchmark_async(name: str, func, iterations: int = 1000, warmup: int = 50) -> BenchmarkResult:
...
def print_comparison(baseline: BenchmarkResult, optimized: BenchmarkResult):
    ...
    improvement = ((baseline.avg_time_us - optimized.avg_time_us) / baseline.avg_time_us) * 100
...
def run_json_benchmarks():
    ...
    for name, data in [("small", SAMPLE_SMALL), ("medium", SAMPLE_MEDIUM), ("large", SAMPLE_LARGE)]:
        print(f"\n[{name.upper()} payload: ~{len(json.dumps(data))} bytes]")
        
        # Baseline: stdlib json
        baseline = benchmark(
            f"json.dumps ({name})",
            lambda d=data: json.dumps(d),
            iterations=10000
        )
...
def run_serializer_benchmarks():
    ...
    for name, data in [("small", SAMPLE_SMALL), ("medium", SAMPLE_MEDIUM)]:
        ...
        print(f"  Size: pickle={pickle_size} bytes, msgpack={msgpack_size} bytes ({(1 - msgpack_size/pickle_size)*100:.1f}% smaller)")
...
def run_sse_benchmarks():
    ...
    baseline = benchmark("SSE with json", lambda: sse_json(event_data), iterations=50000)
    optimized = benchmark("SSE with orjson", lambda: sse_orjson(event_data), iterations=50000)
...
async def run_coalescing_benchmarks():
    ...
    from backend.core.optimized.request_coalescing import (
        RequestCoalescer, CoalesceTaskType, compute_fingerprint
    )
...
def run_lookup_benchmarks():
    ...
    baseline = benchmark(
        "list lookup",
        lambda: test_path in paths_list,
        iterations=100000
    )
    
    optimized = benchmark(
        "frozenset lookup",
        lambda: test_path in paths_frozenset,
        iterations=100000
    )
    ...
    baseline = benchmark(
        ...
        lambda: list(headers_dict.items()),
        iterations=100000
    )
    
    optimized = benchmark(
        "tuple iteration",
        lambda: list(headers_tuple),
        iterations=100000
    )

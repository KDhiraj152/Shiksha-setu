...
def benchmark_numpy_simd(dim: int = 1024, n_vectors: int = 10000, iterations: int = 10) -> Dict[str, float]:
    """Benchmark NumPy SIMD-optimized operations."""
    print(f"\n{'='*60}")
    print(f"NUMPY SIMD BENCHMARK")
    ...
    print(f"Batch MatMul ({batch_size}x): {avg_time*1000:.3f}ms, {throughput:.2f}M ops/sec")
    
    return results


def benchmark_mps_gpu(dim: int = 1024, n_vectors: int = 10000, iterations: int = 10) -> Optional[Dict[str, float]]:
    ...
    print(f"\n{'='*60}")
    print(f"APPLE MPS GPU BENCHMARK")
    print(f"Dimensions: {dim}, Vectors: {n_vectors}, Iterations: {iterations}")
    print(f"{'='*60}")
    ...
    print(f"GPU Batch ({batch_size}x): {avg_time*1000:.3f}ms, {throughput:.2f}M ops/sec")
...
def benchmark_mlx_ane(dim: int = 1024, n_vectors: int = 10000, iterations: int = 10) -> Optional[Dict[str, float]]:
    ...
    print(f"\n{'='*60}")
    print(f"MLX BENCHMARK (Metal/ANE)")
    print(f"Dimensions: {dim}, Vectors: {n_vectors}, Iterations: {iterations}")
    print(f"{'='*60}")
    ...
    print(f"MLX Batch ({batch_size}x): {avg_time*1000:.3f}ms, {throughput:.2f}M ops/sec")
    
    return results


def benchmark_memory_operations(dim: int = 1024, n_vectors: int = 10000) -> Dict[str, float]:
    ...
    print(f"Zero-Copy View: {avg_view:.3f}μs vs Copy: {avg_copy:.1f}μs ({speedup:.0f}x faster)")
...
def benchmark_quantization(dim: int = 1024, n_vectors: int = 10000) -> Dict[str, float]:
    ...
    print(f"\n{'='*60}")
    print(f"QUANTIZATION BENCHMARK")
    print(f"{'='*60}")
    ...
    print(f"Memory: {mem_f32:.1f}MB (f32) vs {mem_f16:.1f}MB (f16) = {results['mem_savings_pct']:.0f}% savings")
...
def run_all_benchmarks():
    ...
    if "numpy_simd" in all_results:
        print(f"  NumPy SIMD Cosine: {all_results['numpy_simd']['cosine_sim_throughput_M']:.2f}M/s")
    if "mps_gpu" in all_results:
        print(f"  MPS GPU MatVec:    {all_results['mps_gpu']['gpu_matvec_throughput_M']:.2f}M/s")
        print(f"  MPS GPU Float16:   {all_results['mps_gpu']['gpu_f16_throughput_M']:.2f}M/s")
    if "mlx" in all_results:
        print(f"  MLX MatVec:        {all_results['mlx']['mlx_matvec_throughput_M']:.2f}M/s")
    
    print("\n[MEMORY PERFORMANCE]")
    if "memory" in all_results:
        print(f"  Copy Bandwidth:    {all_results['memory']['copy_bandwidth_GBs']:.1f} GB/s")

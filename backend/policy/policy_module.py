...
class PolicyMode(str, Enum):
    ...
    RESTRICTED = "restricted"           # Full policy enforcement (default)
...
class PolicyConfig:
    ...
    @classmethod
    def from_env_and_file(cls, config_path: Optional[Path] = None) -> "PolicyConfig":
        """Load configuration from environment variables and config file."""
        config = cls()
        
        # Load from environment variables (highest priority)
        config.allow_unrestricted_mode = os.getenv(
            "ALLOW_UNRESTRICTED_MODE", "false"
        ).lower() == "true"
        
        config.policy_filters_enabled = os.getenv(
            "POLICY_FILTERS_ENABLED", "true"
        ).lower() == "true"
        
        config.sensitive_response_blocking = os.getenv(
            "SENSITIVE_RESPONSE_BLOCKING", "true"
        ).lower() == "true"
        
        config.curriculum_enforcement = os.getenv(
            "CURRICULUM_ENFORCEMENT", "true"
        ).lower() == "true"
        
        config.grade_level_adaptation = os.getenv(
            "GRADE_LEVEL_ADAPTATION", "true"
        ).lower() == "true"
        
        config.allow_external_calls = os.getenv(
            "ALLOW_EXTERNAL_CALLS", "false"
        ).lower() == "true"
        
        config.external_call_audit = os.getenv(
            "EXTERNAL_CALL_AUDIT", "true"
        ).lower() == "true"
        
        config.block_harmful_content = os.getenv(
            "BLOCK_HARMFUL_CONTENT", "true"
        ).lower() == "true"
        
        config.detect_jailbreaks = os.getenv(
            "DETECT_JAILBREAKS", "true"
        ).lower() == "true"
        
        config.redact_secrets = os.getenv(
            "REDACT_SECRETS", "true"
        ).lower() == "true"
        
        config.redact_pii = os.getenv(
            "REDACT_PII", "true"
        ).lower() == "true"
        ...
        if config_path and config_path.exists():
            try:
                ...
                if "ncert_alignment_threshold" in curriculum:
                    config.ncert_alignment_threshold = curriculum["ncert_alignment_threshold"]
...
class PolicyEngine:
    ...
    def mode(self) -> PolicyMode:
        ...
        if self.config.allow_unrestricted_mode:
            if self.config.allow_external_calls:
                return PolicyMode.EXTERNAL_ALLOWED
            return PolicyMode.UNRESTRICTED
    ...
    def _ensure_initialized(self):
        ...
        with self._lock:
            ...
            default_harmful = [
                ...
                {"pattern": r"(?i)\b(how to|ways to|instructions for)\s+(kill|harm|hurt|attack|murder)\b", "risk": "critical", "origin": "safety.py:218"},
                {"pattern": r"(?i)\b(make|build|create)\s+(bomb|explosive|weapon)\b", "risk": "critical", "origin": "safety.py:219"},
                # Illegal activities
                {"pattern": r"(?i)\b(hack|crack|steal)\s+(password|account|credit card)\b", "risk": "high", "origin": "safety.py:222"},
                {"pattern": r"(?i)\b(create|generate)\s+(malware|virus|ransomware)\b", "risk": "critical", "origin": "safety.py:223"},
            ]
            ...
            default_jailbreak = [
                ...
                {"pattern": r"(?i)(ignore|forget|disregard)\s+(previous|all)\s+(instructions|rules|guidelines)", "risk": "high", "origin": "safety.py:226"},
                {"pattern": r"(?i)pretend\s+you\s+(are|can|have)\s+no\s+(restrictions|limits|rules)", "risk": "high", "origin": "safety.py:227"},
                {"pattern": r"(?i)DAN\s+mode|do\s+anything\s+now", "risk": "high", "origin": "safety.py:228"},
            ]
            ...
            for pattern_str in self.config.educational_context_patterns or default_educational:
                try:
    ...
    def apply_input_policy(self, text: str, context: Optional[Dict[str, Any]] = None) -> PolicyCheckResult:
        """
        ...
        if len(text) > self.config.max_input_length:
            filtered_text = text[:self.config.max_input_length] + "\n[Content truncated]"
    ...
    def apply_output_policy(self, text: str, context: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def check_curriculum_alignment(
        ...
        if self.config.allow_unrestricted_mode or not self.config.curriculum_enforcement:
            return PolicyCheckResult(
                allowed=True,
                blocked=False,
        ...
        if grade_level is not None:
            ...
            if not (min_grade <= grade_level <= max_grade):
                return PolicyCheckResult(
                    allowed=False,
                    blocked=True,
                    issues=[f"Grade level {grade_level} outside allowed range {min_grade}-{max_grade}"],
    ...
    def can_make_external_call(self, endpoint: str) -> bool:
        ...
        if not self.config.allow_external_calls:
            logger.warning(f"External call to {endpoint} blocked (ALLOW_EXTERNAL_CALLS=false)")
            return False
    ...
    def _apply_redaction(self, text: str) -> Tuple[str, List[Dict]]:
        ...
        for pattern_str, secret_type in secret_patterns:
            ...
            for match in reversed(matches):
                ...
                result = result[:match.start()] + f"[REDACTED_{secret_type}]" + result[match.end():]
        ...
        if self.config.redact_pii:
            ...
            for pattern_str, pii_type in pii_patterns:
                ...
                for match in reversed(matches):
                    ...
                    result = result[:match.start()] + f"[REDACTED_{pii_type}]" + result[match.end():]

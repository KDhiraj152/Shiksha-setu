...
class RedisCache:
    ...
    def _hash_key(self, content: str) -> str:
        """Create hash-based key from content."""
...
async def cache_get(key: str, cache_type: str = "out") -> Optional[Any]:
    """Quick cache get."""
    cache = await get_cache()
    return await cache.get(key, CacheType(cache_type))


async def cache_set(key: str, value: Any, cache_type: str = "out", ttl: int = None) -> bool:
    """Quick cache set."""
    cache = await get_cache()

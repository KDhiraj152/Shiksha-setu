...
class StudentProgress(Base):
    ...
    content_id = Column(UUID(as_uuid=True), ForeignKey('processed_content.id', ondelete='CASCADE'), index=True, nullable=False)
...
class QuizScore(Base):
    ...
    def __repr__(self):
        return f"<QuizScore(user={self.user_id}, quiz={self.quiz_id}, score={self.score})>"
...
class LearningSession(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), index=True, nullable=False)
    content_id = Column(UUID(as_uuid=True), ForeignKey('processed_content.id', ondelete='CASCADE'), index=True, nullable=True)  # Nullable
    ...
    def __repr__(self):
        return f"<LearningSession(user={self.user_id}, duration={self.duration_seconds}s)>"
...
class ParentReport(Base):
    ...
    parent_id = Column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), index=True, nullable=False)

...
from ...utils.streaming import smart_upload_buffer, stream_file_to_storage, MEMORY_THRESHOLD
import io
...
async def upload_file(
    ...
    Memory-optimized: streams large files to disk instead of loading into memory.
    """
    buffer = None
    try:
        ...
        # Stream file with smart buffering (small files stay in memory)
        buffer = await smart_upload_buffer(file, max_size=max_size)
        
        # Read content for validation (safe - either from memory or temp file)
        ...
        logger.info(f"File uploaded: {file_path} (size: {buffer.size}, in_memory: {buffer.is_in_memory})")

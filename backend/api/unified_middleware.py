...
_HSTS_HEADER_BYTES: tuple[bytes, bytes] = (b"strict-transport-security", b"max-age=31536000; includeSubDomains")
...
_SKIP_ALL_PATHS = frozenset({"/", "/health", "/api/v2/health", "/metrics", "/favicon.ico", "/docs", "/redoc", "/openapi.json"})

# Paths exempt from rate limiting
_RATE_LIMIT_EXEMPT = frozenset({
    "/health", "/docs", "/redoc", "/openapi.json", 
    "/metrics", "/api/v2/health", "/"
})
...
class UnifiedMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if elapsed > self._slow_threshold:
            logger.warning(f"Slow request: {scope.get('method', 'GET')} {path} took {elapsed:.2f}s")
    ...
    def _cleanup_rate_limit_store(self, now: float) -> None:
        ...
        stale_keys = [
            k for k, v in self._rate_limit_store.items()

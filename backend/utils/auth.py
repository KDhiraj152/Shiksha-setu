...
if len(SECRET_KEY) < 64:
    logger.error(f"JWT_SECRET_KEY too short: {len(SECRET_KEY)} chars (minimum 64 required)")
...
def verify_token(token: str, token_type: str = "access") -> Optional[TokenData]:
    ...
    try:
        ...
        email: str = payload.get("email") or payload.get("sub")  # 'sub' often contains email
...
def create_tokens(user_id: str, email: str, role: str = "user") -> Token:
    ...
    refresh_token_encoded, refresh_jti, refresh_expire = create_refresh_token(token_data)

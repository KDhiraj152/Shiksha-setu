...
def record_inference_latency(
    ...
    _metrics["inference_latency"].labels(
    ...
    if latency_seconds > _config.slow_request_threshold:
        logger.warning(
            f"Slow inference: model={model} task={task} "
...
def record_tokens(model: str, input_tokens: int, output_tokens: int):
    ...
    _metrics["tokens_processed"].labels(model=model, direction="input").inc(input_tokens)
    _metrics["tokens_processed"].labels(model=model, direction="output").inc(output_tokens)
...
def record_error(model: str, task: str, error_type: str):
    ...
    _metrics["errors_total"].labels(
        model=model, task=task, error_type=error_type
    ).inc()
    ...
    try:
        ...
        if torch.cuda.is_available():
            for i in range(torch.cuda.device_count()):
                ...
                _metrics["gpu_memory_bytes"].labels(device=f"cuda:{i}", type="used").set(allocated)
                _metrics["gpu_memory_bytes"].labels(device=f"cuda:{i}", type="total").set(total)
    except ImportError:
        pass
...
def track_inference(model: str, task: str):
    ...
    Usage:
        with track_inference("llama-3.2-3b", "simplify"):
...
def track_latency(model: str, task: str):
    ...
    Usage:
        @track_latency("llama-3.2-3b", "simplify")

...
class EvictionAlertHandler(AlertHandler):
    ...
    async def handle(self, alert: Alert) -> bool:
        ...
        if alert.alert_type not in [AlertType.RAM_CRITICAL, AlertType.GPU_CRITICAL, AlertType.OOM_IMMINENT]:
            return True
        ...
        try:
            if self.lifecycle_manager:
                ...
                # Try to import and use global lifecycle manager
                from backend.core.model_lifecycle import get_lifecycle_manager
                manager = get_lifecycle_manager()
                await manager.evict_idle_models()
...
class OOMAlertManager:
    ...
    def __init__(self, config: Optional[OOMAlertConfig] = None):
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            "ram_percent": ram,
            "ram_status": self._get_status(ram, 
                self.config.ram_warning_threshold,
                self.config.ram_critical_threshold),
            "gpu_percent": gpu,
            "gpu_status": self._get_status(gpu,
                self.config.gpu_warning_threshold,
                self.config.gpu_critical_threshold) if gpu else "unavailable",
            "thresholds": {
    ...
    def _get_status(self, value: Optional[float], warning: float, critical: float) -> str:
...
_alert_manager: Optional[OOMAlertManager] = None

...
class CurriculumValidationService:
    ...
    async def validate_content_against_curriculum(
        ...
        """
        logger.info(f"Validating content {content_id} against curriculum (Grade {grade_level}, {subject})")
        
        # Get relevant curriculum standards
        standards = self.db.query(NCERTStandard).filter(
            NCERTStandard.grade_level == grade_level,
            NCERTStandard.subject == subject
        ).all()
        
        if not standards:
            logger.warning(f"No curriculum standards found for Grade {grade_level}, {subject}")
            ...
            validation = ContentValidation(
                ...
                passed=False,
                issues_found={
                    "errors": [f"No curriculum standards available for Grade {grade_level}, {subject}"],
                    "warnings": [],
                    "suggestions": ["Add curriculum standards for this grade/subject combination"]
                }
            )
    ...
    async def validate_factual_accuracy(
        ...
        result = await self.validator.check_factual_accuracy(
            text=text,
            subject=subject
        )
    ...
    async def comprehensive_validation(
        ...
        avg_score = sum(v.alignment_score for v in validations.values()) / len(validations)
    ...
    def get_validation_summary(self, content_id: str) -> Dict[str, Any]:
        ...
        validations = self.db.query(ContentValidation).filter(
        ...
        if not validations:
            return {
                "validated": False,
                "message": "No validation records found"
            }
        ...
        summary["overall_score"] = sum(
            v.alignment_score for v in validations
        ) / len(validations)
    ...
    def get_improvement_suggestions(self, content_id: str) -> List[str]:
        ...
        validations = self.db.query(ContentValidation).filter(
            ContentValidation.content_id == content_id,
            ContentValidation.passed == False
        ).all()

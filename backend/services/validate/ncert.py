...
from ..embeddings.bge_embeddings import BGEM3Embeddings

logger = logging.getLogger(__name__)


class EmbeddingClient:
    """Wrapper client for embedding operations using BGE-M3."""
    
    def __init__(self):
        self._service = None
    
    def _get_service(self) -> BGEM3Embeddings:
        if self._service is None:
            self._service = BGEM3Embeddings()
        return self._service
    
    async def get_embedding(self, text: str) -> np.ndarray:
        """Get embedding for text using BGE-M3."""
        service = self._get_service()
        result = await service.embed_async(text)
        return result.embeddings[0]
    
    async def get_embeddings(self, texts: List[str]) -> List[np.ndarray]:
        """Get embeddings for multiple texts."""
        service = self._get_service()
        result = await service.embed_async(texts)
...
class NCERTValidator:
    ...
    def __init__(self, model_client: Optional[EmbeddingClient] = None):
        """
        Initialize NCERT Validator.
        
        Args:
            model_client: EmbeddingClient instance for embeddings (uses BGE-M3)
        """
        self.model_client = model_client or EmbeddingClient()
        self.standards_loader = NCERTStandardsLoader(self.model_client)

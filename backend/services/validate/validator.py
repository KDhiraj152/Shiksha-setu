"""Validation Module for content quality assurance using BGE-M3 embeddings."""
import re
import math
import logging
...
from .standards import NCERTStandardsLoader, NCERTStandardData
from .ncert import EmbeddingClient
...
class ValidationModule:
    """BGE-M3 based validation module for educational content quality assurance."""
    
    def __init__(self, embedding_client: Optional[EmbeddingClient] = None):
        self.embedding_client = embedding_client or EmbeddingClient()
        self.ncert_loader = NCERTStandardsLoader(self.embedding_client)
    ...
    def validate_content(
        ...
        # 1. Semantic accuracy validation (minimum 0.8 required)
    ...
    def _validate_semantic_accuracy(self, original_text: str, translated_text: str) -> float:
        ...
        try:
            ...
            async def get_similarity():
                emb1 = await self.embedding_client.get_embedding(original_text)
                emb2 = await self.embedding_client.get_embedding(translated_text)
                # Cosine similarity
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(asyncio.run, get_similarity())
                        similarity = future.result()
                else:
                    similarity = loop.run_until_complete(get_similarity())
            except RuntimeError:
                similarity = asyncio.run(get_similarity())
    ...
    def _determine_overall_status(
        ...
        if semantic_score >= 0.80 and age_appropriate:
            return 'passed'

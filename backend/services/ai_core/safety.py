...
class SafetyCheckResult:
    ...
    secrets_found: Optional[List[SecretMatch]] = None
...
class SecretScanner:
    ...
    PATTERNS = {
        SecretType.AWS_KEY: [
            ...
            (r'(?i)aws[_-]?secret[_-]?access[_-]?key["\']?\s*[:=]\s*["\']?([A-Za-z0-9/+=]{40})', RiskLevel.CRITICAL),
        ],
        SecretType.API_KEY: [
            (r'(?i)(api[_-]?key|apikey)["\']?\s*[:=]\s*["\']?([A-Za-z0-9_\-]{20,})', RiskLevel.HIGH),
            (r'(?i)bearer\s+[A-Za-z0-9_\-\.]+', RiskLevel.HIGH),
            (r'sk-[A-Za-z0-9]{48}', RiskLevel.CRITICAL),  # OpenAI API key
            (r'ghp_[A-Za-z0-9]{36}', RiskLevel.CRITICAL),  # GitHub personal access token
            (r'gho_[A-Za-z0-9]{36}', RiskLevel.CRITICAL),  # GitHub OAuth token
        ],
        SecretType.PASSWORD: [
            (r'(?i)(password|passwd|pwd)["\']?\s*[:=]\s*["\']?([^\s"\']{8,})', RiskLevel.CRITICAL),
            (r'(?i)secret["\']?\s*[:=]\s*["\']?([^\s"\']{8,})', RiskLevel.HIGH),
        ],
        SecretType.TOKEN: [
            (r'(?i)(token|auth_token|access_token)["\']?\s*[:=]\s*["\']?([A-Za-z0-9_\-\.]{20,})', RiskLevel.HIGH),
            (r'eyJ[A-Za-z0-9_-]*\.eyJ[A-Za-z0-9_-]*\.[A-Za-z0-9_-]*', RiskLevel.HIGH),  # JWT
        ],
        SecretType.PRIVATE_KEY: [
            (r'-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----', RiskLevel.CRITICAL),
        ...
        SecretType.CREDIT_CARD: [
            (r'\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9][0-9])[0-9]{12})\b', RiskLevel.CRITICAL),
        ],
        SecretType.SSN: [
        ...
        SecretType.PHONE: [
            (r'\b(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b', RiskLevel.MEDIUM),
        ],
    }
    
    def __init__(self):
        # Compile patterns for efficiency
        self._compiled_patterns: Dict[SecretType, List[Tuple[re.Pattern, RiskLevel]]] = {}
        for secret_type, patterns in self.PATTERNS.items():
            self._compiled_patterns[secret_type] = [
                (re.compile(pattern), risk)
...
class SafetyGuard:
    ...
    HARMFUL_PATTERNS = [
        # Violence
        (r'(?i)\b(how to|ways to|instructions for)\s+(kill|harm|hurt|attack|murder)\b', RiskLevel.CRITICAL),
        (r'(?i)\b(make|build|create)\s+(bomb|explosive|weapon)\b', RiskLevel.CRITICAL),
        
        # Illegal activities
        (r'(?i)\b(hack|crack|steal)\s+(password|account|credit card)\b', RiskLevel.HIGH),
        (r'(?i)\b(create|generate)\s+(malware|virus|ransomware)\b', RiskLevel.CRITICAL),
        
        # Jailbreak attempts
        (r'(?i)(ignore|forget|disregard)\s+(previous|all)\s+(instructions|rules|guidelines)', RiskLevel.HIGH),
        (r'(?i)pretend\s+you\s+(are|can|have)\s+no\s+(restrictions|limits|rules)', RiskLevel.HIGH),
    ...
    def __init__(self):
        ...
        self._harmful_patterns = [
            (re.compile(pattern), risk)
            for pattern, risk in self.HARMFUL_PATTERNS
        ]
    ...
    def check_input(self, text: str) -> SafetyCheckResult:
        ...
        if policy and policy.mode == PolicyMode.UNRESTRICTED:
            ...
            secrets = self.secret_scanner.scan(text) if policy.config.redact_secrets else []
        ...
        if secrets:
            critical_secrets = [s for s in secrets if s.risk_level == RiskLevel.CRITICAL]
            if critical_secrets:
                issues.append(f"Critical secrets detected in input ({len(critical_secrets)} found)")

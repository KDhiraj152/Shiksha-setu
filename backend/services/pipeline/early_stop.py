...
class EarlyStopDetector:
    ...
    def __init__(self, config: Optional[EarlyStopConfig] = None, task: str = "simplify"):
        ...
        self._incomplete_patterns = re.compile(r'[,;:]$|(?:and|but|or|so|yet|because|although|while|if|when)$', re.IGNORECASE)
    ...
    def _should_early_stop(self) -> bool:
        ...
        if self.state.consecutive_complete_sentences >= self.config.consecutive_sentences_threshold:
...
class StreamingEarlyStopCallback:
    ...
    def __init__(self, detector: EarlyStopDetector, on_stop: Optional[Callable] = None):

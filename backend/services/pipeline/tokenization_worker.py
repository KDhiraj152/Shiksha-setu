...
class TokenizationConfig:
    ...
    max_length_llm: int = 4096  # Llama-3.2 context
    ...
    async def get_tokenizer(self, tokenizer_type: TokenizerType) -> Any:
        ...
        if tokenizer_type not in self._tokenizers:
            async with self._lock:
                ...
                if tokenizer_type not in self._tokenizers:
                    self._tokenizers[tokenizer_type] = await self._load_tokenizer(tokenizer_type)
...
class PreTokenizationWorker:
    ...
    def _compute_hash(self, text: str, tokenizer_type: TokenizerType) -> str:
        """Compute cache key hash for text."""
    ...
    async def validate_length(
        ...
        return (
            tokenized.token_count <= max_length,
    ...
    def _update_stats(self, elapsed_ms: float, token_count: int) -> None:
        ...
        self._stats["avg_tokenization_time_ms"] = self._total_time_ms / max(total_ops, 1)
...
_worker: Optional[PreTokenizationWorker] = None
